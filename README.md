...
1. **Патч региональных проверок (Eligibility Gate)**: Снятие внутренних проверок доступности аккаунта и геолокации в исполняемом файле `agy` / `agy.exe`. Включает прямое патчирование машинного кода в секции `.text` для версий `1.1.17`, `1.1.18` и новее (`EligibilityError`, `IneligibilityFromResult`) и байт-патчинг дескрипторов для ранних версий (`1.1.9`).
...
| **Поддерживаемые версии CLI** | `1.1.9`, `1.1.16`, `1.1.17`, `1.1.18` (протестировано) и новее — включая машинный патч Go-рантайма |
